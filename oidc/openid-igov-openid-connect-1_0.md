...
profile for iGov **in the Netherlands ("NL-iGov")**.
...
When an **NL**-iGov-compliant component is interacting with other **NL**-iGov-compliant
components, in any valid combination, all components MUST fully conform to the
features and requirements of this specification. All interaction with non-**NL**-iGov
components is outside the scope of this specification.

An **NL**-iGov-compliant OpenID Connect IdP MUST support all features as described in
this specification. A general-purpose IdP MAY support additional features for
use with non-**NL**-iGov clients.

An **NL**-iGov-compliant OpenID Connect IdP MAY also provide iGov-compliant OAuth 2.0
authorization server functionality. In such cases, the authorization server
MUST fully implement the OAuth 2.0 **NL**-iGov profile. If an iGov-compliant OpenID
...
An **NL**-iGov-compliant OpenID Connect client MUST use all functions as described in
this specification. A general-purpose client library MAY support additional
features for use with non-**NL**-iGov IdPs.

#  2. Relying Party Profile

##  2.1. Requests to the Authorization Endpoint (Authentication Request)

The **NL**-iGov OAuth2 profile specifies requirements for requests to Authorization
Endpoints - for example, when to use the PKCE parameters to secure token
exchange.

In addition to the requirements specified in Section ~~2.1.1~~ **ref** of the **NL**-iGov OAuth2
profile, the following describes the supported OpenID Connect Authorization
Code Flow parameters for use with **NL**-iGov compatible IdPs.
...
**NL-iGov**
vtr has been excluded from the NL-iGov profile, as Europe uses level-of-assurance (LoA) policies which fit best in acr.
**/NL-iGov**
~~vtr~~
...
In addition to the requirements specified in Section ~~2.1.2~~ **ref** of the **NL**-iGov OAuth2
...
**NL-iGov**
`vot` and `vtm` have been excluded from the NL-iGov profile, as Europe uses level-of-assurance (LoA) policies which fit best in acr.
**/NL-iGov**
~~vot~~
...
**NL-iGov**
Vectors of trust have been excluded from the NL-iGov profile, as Europe uses level-of-assurance (LoA) policies which fit best in acr. Although vectors of trust provide more granularity and support for some anonymous use cases, these are less common in citizen-to-government use cases and therefor not included.
**/NL-iGov**
