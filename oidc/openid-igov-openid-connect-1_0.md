...
iss
    ...
    The "issuer" field is the Uniform Resource Locator (URL) of the expected issuer 
aud
...
acr
    ...
    Clients SHOULD validate the authentication context class reference, if present, satisfies the minimum required before authorizing access to any resource or performing any operation on behalf of the identified subject.
...
iss
    ...
    REQUIRED. The "issuer" field is the Uniform Resource Locater (URL) of the OpenID Provider. 
aud
...
In case the request objects using pass by reference are supported, registration of `request_uri` is required.
