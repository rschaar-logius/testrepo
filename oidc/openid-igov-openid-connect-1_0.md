...
client_assertion
    ...
    **REQUIRED.** The value of the signed client authentication JWT generated as described below. The RP **MUST** generate a new **client**_assertion JWT for each call to the token endpoint. 
...
code_verifier
    ...
    ~~OPTIONAL. If~~ **REQUIRED in case** the PKCE protocol was used by the Client in the relevant Authentication Request, the code_verifier MUST be present and the value MUST be the original random code, used to create the hashed challenge in `code_challenge`.
...
acr
    ...
    Client**s** MUST validate the authentication context class reference, if present, satisfies the minimum required before authorizing access to any resource or performing any operation on behalf of the identified subject.
...
iss
    ...
    REQUIRED. The "issuer" field is the Uniform Resource Locat~~e~~**o**r (URL) of the ~~expected issuer~~ **OpenID Provider**. 
aud
...
acr_values_**supported**
    ...
    OPTIONAL. The acr** value**s supported. 
